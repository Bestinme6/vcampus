...
class LibraryViewDataTest {
    @Test void parsesCatalogPageInServerFieldOrder(){Map<String,String>d=page();d.put("row.0",RowCodec.encode("10","BK000000010","9787111565277","Java 并发编程实战","Brian Goetz","机械工业出版社","2020","计算机","并发编程","true","4","2"));var p=LibraryViewData.catalogPage(ok(d));assertEquals("BK000000010",p.rows().getFirst().catalogCode());assertEquals("Java 并发编程实战",p.rows().getFirst().title());assertEquals(2,p.rows().getFirst().availableCopies());}
    ...
    @Test void rejectsLegacyElevenFieldCatalogRowsSoClientAndServerMustUpgradeTogether(){Map<String,String>d=page();d.put("row.0",RowCodec.encode("10","9787111565277","Java","作者","出版社","2020","计算机","简介","true","1","1"));var e=assertThrows(IllegalArgumentException.class,()->LibraryViewData.catalogPage(ok(d)));assertEquals("服务器返回的图书馆数据格式不正确",e.getMessage());}
    @Test void rejectsNegativePaginationMetadata(){Map<String,String>d=page();d.put("total","-1");d.put("row.0",RowCodec.encode("10","BK000000010","9787111565277","Java","作者","出版社","2020","计算机","简介","true","1","1"));assertThrows(IllegalArgumentException.class,()->LibraryViewData.catalogPage(ok(d)));}
