...
abstract class LibraryPanel extends JPanel {
    ...
    protected void runRequest(RequestCall call,ResponseConsumer consumer){if(busy||!LegacyUiLifecycle.active(this))return;setBusy(true);CompletableFuture.supplyAsync(()->{try{return call.execute();}catch(Exception e){throw new CompletionException(e);}}).whenComplete((r,e)->SwingUtilities.invokeLater(()->{if(!LegacyUiLifecycle.active(this))return;setBusy(false);if(e!=null)error(root(e).getMessage());else if(!r.success())error(r.message());else try{consumer.accept(r);}catch(RuntimeException x){error(x.getMessage());}}));}
