...
public final class SocketShopGateway implements ShopGateway {
    ...
    @Override public int adjustInventory(long productId, int delta, String reason) throws IOException { ResponseMessage response = client.adjustShopInventory(token, productId, delta, reason); ShopData.require(response); return Integer.parseInt(response.data().get("stockAfter")); }
