...
public final class LibraryService {
    ...
    public ResponseMessage searchCatalog(RequestMessage r) { return handle(r, false, s -> catalogPage(r, catalog.search(new CatalogQuery(p(r,"keyword"),p(r,"category"),page(r),PAGE_SIZE)))); }
    ...
    public ResponseMessage createBook(RequestMessage r) { return handleManage(r,s -> { long id=catalog.createBook(book(r)); audited(s,r); return ok(r,"创建成功",Map.of("bookId",Long.toString(id))); }); }
    ...
    public ResponseMessage searchCopies(RequestMessage r) { return handleManage(r,s -> copyPage(r,catalog.searchCopies(new CopyQuery(optionalId(r,"bookId"),p(r,"keyword"),enumValue(r,"status",LibraryCopyStatus.class),page(r),PAGE_SIZE)))); }
    public ResponseMessage createCopy(RequestMessage r) { return handleManage(r,s -> { long id=catalog.createCopy(new CreateCopy(id(r,"bookId"),LibraryCodePolicy.requireValidBarcode(p(r,"barcode")),p(r,"shelfLocation"))); audited(s,r); return ok(r,"创建成功",Map.of("copyId",Long.toString(id))); }); }
    ...
    private BookCommand book(RequestMessage r){return new BookCommand(LibraryCodePolicy.normalizeIsbn(p(r,"isbn")),required(r,"title"),required(r,"authors"),p(r,"publisher"),integer(r,"publishYear"),p(r,"category"),p(r,"description"));}
    ...
    private String catalogRow(CatalogItem x){return RowCodec.encode(Long.toString(x.bookId()),x.isbn(),x.title(),x.authors(),x.publisher(),x.publishYear()==null?"":x.publishYear().toString(),x.category(),x.description(),Boolean.toString(x.enabled()),Integer.toString(x.totalCopies()),Integer.toString(x.availableCopies()));}
