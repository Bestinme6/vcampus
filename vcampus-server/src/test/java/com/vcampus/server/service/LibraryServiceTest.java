...
class LibraryServiceTest {
    ...
    @Test void catalogEncodingHasStableFieldOrder(){var student=session(2,UserRole.STUDENT);var response=service.searchCatalog(request(Actions.LIBRARY_CATALOG_SEARCH,student.token(),Map.of()));assertTrue(response.success());assertEquals(List.of("10","BK000000010","9787111565277","Java","作者","出版社","2026","计算机","简介","true","4","2"),RowCodec.decode(response.data().get("row.0")));}
    ...
    @Test void administratorCanSubmitDamagedReturn(){var admin=session(4,UserRole.LIBRARY_ADMIN);var response=service.adminReturn(request(Actions.LIBRARY_ADMIN_LOAN_RETURN,admin.token(),Map.of("barcode","B000000128","condition","DAMAGED","reason","封面脱落")));assertTrue(response.success());assertEquals(LibraryReturnCondition.DAMAGED,loans.returnCommand.condition());assertEquals("封面脱落",loans.returnCommand.reason());assertTrue(loans.returnCommand.administrator());assertEquals(Actions.LIBRARY_ADMIN_LOAN_RETURN,audit.action);}
