...
- 提供论坛、图书馆和银行来源筛选；论坛详情可打开关联帖子，图书馆和银行详情可打开对应业务页。
...
4. 确认 `notifications` 表的三个检查约束已经包含图书馆、论坛和银行枚举值；
5. 重启应用服务器和客户端。
...
- 强制修改初始密码期间不能访问消息中心；
- 消息跳转使用客户端枚举映射，只能打开教师课表、学生成绩、学籍信息、“我的借阅”、银行流水或关联论坛帖子，不能执行服务器传来的 URL、类名或文件路径；
...
商店接入时也必须在业务事务的同一个 `Connection` 上调用 `NotificationWriter`。禁止业务提交成功后再单独建立连接补发消息，否则会出现业务成功但消息丢失的不一致状态。
