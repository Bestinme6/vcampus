...
- 提供论坛、图书馆、银行和商店来源筛选；论坛详情可打开关联帖子，图书馆和银行详情可打开对应业务页，商店详情可打开关联订单。
...
| 商店订单支付成功 | 订单买家 | `SHOP_ORDER_PAID` | `SHOP` | `SHOP_ORDERS`（关联订单 ID） |
| 商店订单取消并退款 | 订单买家 | `SHOP_ORDER_REFUNDED` | `SHOP` | `SHOP_ORDERS`（关联订单 ID） |
| 商店管理员成功发货 | 订单买家 | `SHOP_ORDER_SHIPPED` | `SHOP` | `SHOP_ORDERS`（关联订单 ID） |

保存成绩草稿不会发送消息，只有“发布最终成绩”才会通知学生；重复的角色配置或启停状态不会生成重复消息。论坛中的自评论、自管理、失败操作和未改变状态的重复操作不会生成通知；作者自行删除内容也不通知自己。用户主动转账不通知自己；充值重试和未改变状态的重复冻结或解冻不产生重复通知。支付与退款通知作为电子订单回执保留，同一结算业务或重复取消不会重复发送；商店扣款不再额外生成银行通知。
...
商店支付、退款和发货均在业务事务的同一个 `Connection` 上调用 `NotificationWriter`；订单、资金、库存和通知一起提交或回滚。禁止业务提交成功后再单独建立连接补发消息，否则会出现业务成功但消息丢失的不一致状态。
...
13. B 结算订单后应收到一条支付回执；取消后应收到一条退款回执；两条消息均能直达对应订单详情，业务重试不得重复通知；
14. 商店管理员为 B 的已支付订单发货，B 应收到商店通知并能直达对应订单详情；重复发货不得重复通知。
