...
- 提供论坛、图书馆、银行和商店来源筛选；论坛详情可打开关联帖子，图书馆、银行和商店详情可打开对应业务页。
...
| 他人成功转账 | 收款人 | `BANK_TRANSFER_RECEIVED` | `BANK` | `BANK_LEDGER` |
| 管理员成功充值 | 目标账户用户 | `BANK_ACCOUNT_TOPPED_UP` | `BANK` | `BANK_LEDGER` |
| 管理员实际冻结或解冻账户 | 目标账户用户 | `BANK_ACCOUNT_STATUS_CHANGED` | `BANK` | `BANK_LEDGER` |
| 商店管理员成功发货 | 订单买家 | `SHOP_ORDER_SHIPPED` | `SHOP` | `SHOP_ORDERS` |

保存成绩草稿不会发送消息，只有“发布最终成绩”才会通知学生；重复的角色配置或启停状态不会生成重复消息。论坛中的自评论、自管理、失败操作和未改变状态的重复操作不会生成通知；作者自行删除内容也不通知自己。用户主动转账不通知自己；充值重试和未改变状态的重复冻结或解冻不产生重复通知。
...
4. 确认 `notifications` 表的三个检查约束已经包含图书馆、论坛、银行和商店枚举值；
5. 重启应用服务器和客户端。
...
- 强制修改初始密码期间不能访问消息中心；
- 消息跳转使用客户端枚举映射，只能打开教师课表、学生成绩、学籍信息、“我的借阅”、银行流水、我的订单或关联论坛帖子，不能执行服务器传来的 URL、类名或文件路径；
- 论坛帖子深链仍通过服务端权限校验；帖子已隐藏或删除且当前用户无权查看时，客户端提示“该帖子当前不可访问”并返回论坛首页；
- 密码重置消息不包含临时密码、密码哈希或盐值。

## 事务规则

排课、成绩发布、学籍状态、账号安全修改、图书馆借还、论坛评论与审核，以及银行转账、充值和状态修改，均在业务自身的 JDBC 事务中通过 `NotificationWriter` 写消息。论坛的内容状态、评论统计、审核日志和通知必须一起提交或一起回滚；银行余额、流水和通知也必须一起提交或一起回滚。

商店发货在业务事务的同一个 `Connection` 上调用 `NotificationWriter`，订单状态与通知一起提交或回滚。禁止业务提交成功后再单独建立连接补发消息，否则会出现业务成功但消息丢失的不一致状态。
...
11. 用户 A 向用户 B 转账，B 应收到银行通知并能直达流水页；A 不收到自己的转账通知；
12. 管理员充值、冻结和解冻 B 的账户，B 应分别收到通知；重复业务编号或状态未变化时不得重复通知。
13. 商店管理员为 B 的已支付订单发货，B 应收到商店通知并能直达“我的订单”；重复发货不得重复通知。
