...
全新数据库依次执行 `database/schema.sql` 和 `database/seed.sql`。已经执行过商店迁移的旧数据库先备份，再执行 `database/migrations/009_shop_receipt_notifications.sql`；尚未安装银行和商店表的旧数据库依次执行 007、008、009，最后执行最新的 `database/seed.sql`。商店结算依赖 007 创建的银行账户与流水结构。
